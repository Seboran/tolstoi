...
describe('GameRules', () => {
  test('should display first part of rules on loading', async () => {
    ...
    expect(wrapper.text()).toContain("Dans ce jeu, tout le monde doit discuter autour d'un débat")
  })
  test('should NOT display second part of rules on loading', async () => {
    const wrapper = mount(GameRules)
    expect(wrapper.text()).not.toContain('Pour jouer à ce jeu, choisissez le nombre de joueurs')
  })
