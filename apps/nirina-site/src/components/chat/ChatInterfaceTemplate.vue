...
<template>
  <div>
    ...
    <form
      class="big-perspective-on-hover chat-container relative top-0 m-auto grid max-w-lg columns-1 flex-row gap-3 bg-chat p-4"
      @submit.prevent="handleFormSubmit"
    >
      <div class="flex w-full flex-row">
        <textarea
          class="h-16 w-full resize-none border-none bg-transparent outline-hidden placeholder:text-slate-400/75 dark:placeholder:text-slate-100/75 focus:placeholder:text-slate-400/50 dark:focus:placeholder:text-slate-100/50"
        ...
        ></textarea>
        <button
          class="bg-chat-button h-8 w-8 rounded-xs p-2"
...
.chat-container {
  ...
  border-image: linear-gradient(
    to bottom right,
    var(--color-chat-border-light) 0%,
    var(--color-chat-border-dark) 25%,
    var(--color-border-gradient-light) 50%,
    var(--color-border-gradient-light) 75%,
    var(--color-border-gradient-light) 100%
  );
...
@media (prefers-color-scheme: dark) {
  .chat-container {
    ...
    border-image: linear-gradient(
      ...
      var(--color-chat-border-light) 0%,
      var(--color-chat-border-dark) 25%,
      var(--color-border-gradient-dark) 50%,
      var(--color-border-gradient-dark) 75%,
      var(--color-border-gradient-dark) 100%
    );
    border-image-slice: 1;
  }
}
